...
const std::string DD_VS = ""
  ...
  "   v_norm = u_nm * a_norm; "
  "   mat4 mv = u_vm * u_mm; "
  "   vec4 light_pos = mv * vec4(0.0, 0.0, 10.0, -100.0);"
...
const std::string DD_FS = ""
  ...
  "   vec3 spec = vec3(0.0);"
  "   if(sdotn > 0.0) { "
  "       spec = vec3(1.0, 1.0, 1.0) * pow(max(dot(r,v), 0.0), 18.0); "
  "   }"
  "  gl_FragColor.a = 1.0;"
  "  gl_FragColor.rgb = max(sdotn,0.01) * (0.5 + 0.5 * v_norm); "
  "  if(u_mode == 1) { "
   "      gl_FragColor.rgb = spec + vec3(0.5, 0.0,0.);"
  "  }"
