...
# -------------------------------------
# Creates static libraries for video recorder addon
# 
# Dependencies
# ------------
...
export CFLAGS="-Os -I${bd}/include -arch i386" 
...
if [ "$1" = "x264" ] ; then 
    ...
    cd x264
    ./configure --prefix=${bd} --enable-static --host=i386-apple-darwin 
    make
    make install
fi
...
if [ "$1" = "av" ] ; then 
    cd libav/libav/
    ./configure --prefix=${bd} --enable-static --arch=x86_32
    make 
    make install
fi
