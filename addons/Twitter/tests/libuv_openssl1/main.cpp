...
void handle_error(Client* c, int result) {
  ...
  if(error == SSL_ERROR_WANT_WRITE) {
    printf("handle_error, WANT WRITE\n");
  }
  else if(error == SSL_ERROR_WANT_READ) { // wants to read from bio
    printf("handle_error, WANT READ\n");
    flush_read_bio(c);
  }
  else {
    printf("handle_error, UNKNOWN: %d\n", error);
  }
}
...
void check_outgoing_application_data(Client* c) {    
  if(SSL_is_init_finished(c->ssl)) {
    if(c->buffer_out.size() > 0) {
      ...
      int to_write = BIO_ctrl_pending(c->write_bio);
      flush_read_bio(c);
    }
  }
}
...
void on_read_callback(uv_stream_t* tcp, ssize_t nread, uv_buf_t buf) {
  ...
  printf("on_read_callback, written %d bytes to bio.\n", nread);
  on_event(c);
}
...
int main() {
  ...
  // This will send the client certificat to the server (== client certificate verification) 
  /*
  ...
  */
  ...
  SSL_CTX_set_options(ssl_ctx, SSL_OP_NO_SSLv2); // | SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION);
  SSL_CTX_set_verify(ssl_ctx, SSL_VERIFY_PEER, dummy_ssl_verify_callback); // NULL); // no verify, testing libuv + openssl mem bios
  //SSL_CTX_set_verify(ssl_ctx, SSL_VERIFY_NONE, NULL); // no verify, testing libuv + openssl mem biosw
  // SSL_CTX_set_verify(ssl_ctx, SSL_VERIFY_PEER, NULL); // no verify, testing libuv + openssl mem bios
  //SSL_CTX_set_verify(ssl_ctx, SSL_VERIFY_CLIENT_ONCE, NULL); // no verify, testing libuv + openssl mem bios
