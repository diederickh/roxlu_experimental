...
TwitterStream::~TwitterStream() {
	...
	{
		deque<TweetURL*>::iterator it = download_twitpics.begin();
	...
	// Remove tweets
	{
		deque<Tweet*>::iterator it = tweets.begin();
		while(it != tweets.end()) {
			delete *it;
			it = tweets.erase(it);
		}
			...
			for(int i = 0; i < num_urls; ++i) {
				...
				if(url != NULL) {
					...
					if(json_is_string(exp_url)) {
						...
						TweetURL* tu = new TweetURL(exp_url_str);
						tweet->addURL(tu);
					}
				}
			}
...
void TwitterStream::removeTweet(Tweet* tweet) {
	deque<Tweet*>::iterator it = std::find(tweets.begin(), tweets.end(), tweet);
	if(it != tweets.end()) {
		tweets.erase(it);
		delete (*it);
	}
...
void TwitterStream::downloadTwitPic(string fileName, TweetURL* url) {
	if(url->isTwitPic()) {
