...
if [ "$1" = "reset" ]; then
	...
	if [ ! -f ${pwd}/download/libpng.tgz ] ; then
		curl ftp://ftp.simplesystems.org/pub/libpng/png/src/libpng-1.5.10.tar.gz -o download/libpng.tgz
	fi
	
	if [ ! -f ${pwd}/download/pixman.tgz ] ; then
		curl http://www.cairographics.org/releases/pixman-0.24.4.tar.gz -o download/pixman.tgz
	fi
	
	if [ ! -f ${pwd}/download/cairo.tgz ] ; then
		curl http://www.cairographics.org/releases/cairo-1.10.2.tar.gz -o download/cairo.tgz
	fi
