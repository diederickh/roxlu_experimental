...
if [ ! -d libav ] ; then
    mkdir libav 
    cd libav 
    git clone git://git.libav.org/libav.git . && git reset --hard 65d94f63ca38b46a9d3719cb7d986d2daef90416
fi
...
if [ "$1" = "av" ] ; then 
    cd libav 
    ./configure --prefix=${bd} --enable-static --enable-libx264 --enable-libspeex --target-os=darwin --enable-gpl
    make 
    make install
fi
