...
namespace gl { 
  ...
  HorizontalBlur::HorizontalBlur(float amount) 
    :blur_amount(amount)
  {
  }

  bool HorizontalBlur::setup(int w, int h) { 

    // CREATE SHADER
    if(!shader.create(HORIZONTAL_BLUR_FILTER_VS, HORIZONTAL_BLUR_FILTER_FS)) {
    ...
    u_width = shader.getUniformLocation("u_width");

    glUniform1f(u_width, w);
  ...
  float HorizontalBlur::gauss(float x, float sigma2) {
